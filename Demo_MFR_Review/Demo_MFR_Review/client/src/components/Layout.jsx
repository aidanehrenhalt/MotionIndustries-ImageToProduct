...
export default function Layout({ activePage, onNavigate, queueCount, historyCount, children }) {
  const navItems = [
    { key: "input", label: "Input UI", icon: <Icons.Upload />, count: queueCount > 0 ? queueCount : null },
    { key: "review", label: "Output UI", icon: <Icons.Grid />, count: queueCount > 0 ? queueCount : null },
    { key: "history", label: "History", icon: <Icons.Clock />, count: historyCount > 0 ? historyCount : null },
  ];
