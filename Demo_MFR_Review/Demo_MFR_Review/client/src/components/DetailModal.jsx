...
export default function DetailModal({ item, onClose }) {
  ...
  const extraAttributes = useMemo(() => Object.entries(item.attributes || {}).slice(0, 20), [item.attributes]);
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9000, display: "flex", justifyContent: "flex-end" }}>
      ...
      <div style={{ position: "relative", width: 580, maxWidth: "95vw", background: COLORS.white, height: "100%", overflowY: "auto", boxShadow: "-8px 0 40px rgba(0,0,0,0.15)", fontFamily: fontStack }}>
        ...
        <div style={{ padding: 24 }}>
          ...
          <p style={{ fontSize: 13, color: COLORS.g500, margin: "0 0 20px", lineHeight: 1.6 }}>{item.description || "No description available."}</p>
          ...
          {extraAttributes.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5, color: COLORS.g400, marginBottom: 10 }}>Parsed JSON Fields</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 18px" }}>
                {extraAttributes.map(([key, value]) => (
                  <div key={key}>
                    <div style={{ color: COLORS.g400, fontSize: 11, marginBottom: 2 }}>{key}</div>
                    <div style={{ color: COLORS.g800, fontWeight: 500 }}>{String(value)}</div>
                  </div>
                ))}
              </div>
            </div>
